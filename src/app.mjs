...
app.post('/article/add', (req, res) => {
  ...
  article.save((err) =>{
    if(!err){
      ...
      res.render('article/add', {message: 'error adding article'}); 
    }
  });
});
...
app.get('/article/:slug', (req, res) => {
  ...
  Article.findOne({title: req.params.slug}).exec((err, article) => {
    res.render('article-detail', {article:article, user:req.session.user});
  });
});
